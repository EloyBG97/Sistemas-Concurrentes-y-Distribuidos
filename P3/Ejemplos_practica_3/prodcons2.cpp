...
void productor(int rank)
{
   ...
   for ( unsigned int value=0; value < ITERS/5 ; value++ )
   { 
      cout << "Productor " << rank << " produce el valor " << value << endl << flush ;
...
void buffer()
{
   ...
   {  
      if ( pos==0 )      // el consumidor no puede consumir
         ...
         rama = 1 ;           
      else               // tanto el consumidor como elproductor pueden actuar
      {
...
void consumidor(int rank)
{
   ...
   for (unsigned int i=0;i<ITERS/4;i++)
   {
      ...
      cout << "Consumidor " << rank << "recibe el valor " << value << " de Buffer " << endl << flush ;
